...
遺伝子 ID システムの利点が最もよく効いているのが文献の層である。著者がコミュニティの命名
規約に従って遺伝子を記載しているため、論文から遺伝子への言及を大規模に収集できる。**約 500
件の論文を 1,200 件以上の遺伝子と結びつけ**、各リンクには evidence (experimental / sequence /
citation) と role (subject / comparator / background) を付し、core か supporting かを区別して
いる。さらに、紐づいた文献をもとに LLM で各遺伝子の機能要約を作成した。リンクも要約も RDF に
含めてあるので、エージェントは論文を読み直すのではなくデータとして取得できる。
